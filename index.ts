...
async function displayStatus() {
  ...
  console.log(
    chalk.bold(
      `Current Enemy: ${chalk.cyan.bold(enemies[currentEnemyIndex].name)}`
    )
  );
  console.log(
    chalk.bold(
      `Enemy Health: ${chalk.cyan.bold(enemies[currentEnemyIndex].health)}`
    )
  );
  console.log(
    chalk.bold(`Energy Potions: ${chalk.cyan.bold(energyPotions)}\n`)
  );
}
...
function usePotion() {
  if (energyPotions > 0) {
    ...
    console.log(
      chalk.bold(
        `\nYou used an energy potion. Health increased to ${chalk.cyan.bold(
          health
        )}.\n`
      )
    );
...
async function startGame() {
  ...
  console.log(
    chalk.bold.hex("#3357FF")(
      "You will encounter various enemies, including Zombie, Skeleton, and Assassin."
    )
  );
  console.log(
    chalk.bold.hex("#3357FF")(
      "Keep an eye on your health and use energy potions wisely to stay alive."
    )
  );
  console.log(
    chalk.bold.hex("#3357FF")(
